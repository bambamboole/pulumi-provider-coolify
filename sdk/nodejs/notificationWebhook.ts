...
/**
 * Generic JSON webhook notifications. Manages one settings object per API token team on Coolify v4.3.0+. Create adopts existing settings. Only declared fields are changed. Delete disables delivery and retains credentials and event choices.
 */
export class NotificationWebhook extends pulumi.CustomResource {
     ...
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, opts?: pulumi.CustomResourceOptions): NotificationWebhook {
        return new NotificationWebhook(name, undefined as any, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'coolify:index:NotificationWebhook';
    ...
    /**
     * webhook enabled. Omit to leave the current setting unmanaged.
     */
    ...
    /**
     * The Coolify team ID derived from the provider's API token. Import using <teamId>/webhook.
     */
    declare public /*out*/ readonly teamId: pulumi.Output<number>;
    /**
     * webhook url. Treated as a Pulumi secret. Omit to leave the current setting unmanaged. An empty string clears the setting.
     */
