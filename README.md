...
- **Service compose files are compared as YAML when the token may read them.** Coolify reports `docker_compose_raw` only to tokens with `read:sensitive` or `root`. With such a token the provider compares the declared `dockerCompose` with Coolify's copy as parsed YAML, because Coolify re-dumps the file it stores and formatting differences must not count as drift. A file changed in the Coolify UI shows up on `pulumi refresh` and is restored on the next update, and adopting a service whose file already matches sends nothing. Without the permission the file is sent on adoption and whenever the input changes, and drift is not detected.
- **Application sources are resolved on read.** Coolify reports the numeric IDs of an application's private key and GitHub App, and the provider maps them to `privateKeyUuid` and `githubAppUuid` for the declared source, so a key or app switched in the UI shows up on `pulumi refresh`. Adopting a `private-github-app` application that uses another GitHub App re-applies the declared one. Adopting a `private-deploy-key` application that uses another key fails with a clear error, because Coolify's API cannot change the key of an existing application. `serverUuid` cannot be resolved, Coolify only reports a destination ID.
...
- **UUIDs stay known in update previews.** An application, service, database or storage keeps its `uuid` (and a storage its `volumeName`) known while other inputs change in a preview; only the inputs whose change replaces the resource make it unknown. Dependents such as a `coolify.Deployment` or a `coolify.Storage` therefore plan an update instead of showing unknown inputs, and resources registered inside an `apply` on the UUID stay in the plan. `coolify.Storage` and `coolify.VolumeBackup` accept owner and storage UUIDs that are still unknown in a preview; the real values are validated on create and update.
